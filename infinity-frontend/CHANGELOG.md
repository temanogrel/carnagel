...
### Improvements
* Added `modals.js` files for blueprints for convenient imports
...
### Improvements
* Renamed application entry point from `modals.js` -> `app.js` (clarifies intent and helps with coverage reports)
...
* `reducers/modals.js` now exports combined root reducer.
