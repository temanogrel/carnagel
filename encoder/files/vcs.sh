...
# Disable header meta info
declare -i DISABLE_META_INFO=0
# Custom filename
declare CUSTOM_FILENAME=""
...
test_programs() {
	...
	if [[ $ver ]]; then
        ...
        error "Failed to check ImageMagick version. Found $ver."
		(( retval++ ,1 ))
	fi
...
csheet_overlap() {
	...
	# previous one. Note it is important to work over $AS and not $VID[$W]
...
get_vcodec_name() {
	...
	case "$vcid" in
		...
		I420) vcodec="Raw I420 Video" ;; # XXX: Officially I420 is Indeo 4 but it is mapped to raw Â¿?
...
process() {
	...
	if [[ $CUSTOM_FILENAME ]]; then
	    filename_value="$CUSTOM_FILENAME.mp4"
	fi

    inf "$filename_value"

	if [[ $DISABLE_META_INFO -eq 0 ]]; then
		convert \
				\( \
				...
				"$output" -append \
				\( \
					-size ${headwidth}x0 -gravity Center "xc:$BG_SIGN" \
					-font "$FONT_SIGN" -pointsize "$PTS_SIGN" \
					-fill "$FG_SIGN" -annotate 0 "" \
				\) \
				-append \
				"$output"
	fi
...
Options:
  --filename <arg>      Set custom filename in meta info
...
"fullhelp,profile:,"\
"jpeg2,nonlatin,generate:,config:,filename:," \
       -- "$@")
eval set -- "$TEMP"
...
while true ; do
	case $1 in
		...
		--filename) CUSTOM_FILENAME="$2" ; shift ;;
		-f|--from)
		...
		-d|--disable) # Disable default features
			case $(tolower "$2") in
				# disable meta info in resulting image (no header / footer)
				metainfo)
					if [[ $DISABLE_META_INFO -eq 0 ]]; then
						inf "Meta info disabled."
						DISABLE_META_INFO=1
					fi
					;;
