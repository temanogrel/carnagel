...
type mp42hlsWorker struct {
	app    *encoder.Application
	logger logrus.FieldLogger
}

func NewMp42hlsWorker(application *encoder.Application) encoder.Worker {
	return &mp42hlsWorker{
		app:    application,
		logger: application.Logger.WithField("component", "worker"),
	}
}

func (worker *mp42hlsWorker) Run(ctx context.Context, routines int) {
	...
	consumingChannel, err := ch.Consume(ecosystem.AmqpQueueMp42Hls, worker.app.Hostname, false, false, false, false, nil)
	if err != nil {
...
func (worker *mp42hlsWorker) processMessages(ctx context.Context, ch <-chan amqp.Delivery) {
	...
	for {
		...
		default:
			...
			worker.app.Logger.Debug("Received a new recording convert mp42hls")

			payload := ecosystem.RecordingPayload{}
			...
			if err := worker.app.Pipelines.ConvertMp42Hls(ctx, recording); err != nil {
				worker.app.Logger.
					WithError(err).
